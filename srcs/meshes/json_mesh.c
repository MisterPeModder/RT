...
/*   Updated: 2018/08/14 08:15:37 by yguaye           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int			parse_json_face(t_face *face, const t_json_value *data)
{
	...
	ft_memset(face->normals, -1, data->arr.values_num);
	i = 0;
	while (i < data->arr.values_num)
	{
		...
		const t_json_value *data)
{
	...
	while (i < data->arr.values_num)
	{
		if (!parse_json_face(&faces[i], data->arr.values[i]))
		{
			while (i > 0)
			{
