...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/06/07 15:30:46 by achaussa          #+#    #+#             */
/*   Updated: 2018/06/07 15:30:49 by achaussa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int 		seed = 20;
static int 		hash[] = {208,34,231,213,32,248,233,56,161,78,24,140,71,48,140,254,245,255,
...
float			noise(void)
{
	...
	x = 0;
	y = 0;
	if (!(tab = (float**)malloc((sizeof(float*) * 45 + 1))))
		return (0);
	while (y < 45)
	{
		x = 0;
		if (!(tab[y] = (float*)malloc((sizeof(float) * 45 + 1))))
			return (0);
		while (x < 45)
		{
			tab[y][x] = perlin2d(x, y, 0.1, 4);
			//printf("%.1f ", tab[y][x]);
			x++;
		}
		//printf("\n");
