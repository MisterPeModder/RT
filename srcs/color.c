...
/*   Updated: 2018/05/13 17:42:38 by yguaye           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_vec3f *light_color)
{
	...
	r = (float)color[0] + (s_color.x * comp[0] + comp[1]) * light_color->x * 255;
	g = (float)color[1] + (s_color.y * comp[0] + comp[1]) * light_color->y * 255;
	b = (float)color[2] + (s_color.z * comp[0] + comp[1]) * light_color->z * 255;
	color[0] = ((r > 255) ? 255 : r);
	color[1] = ((g > 255) ? 255 : g);
	color[2] = ((b > 255) ? 255 : b);
}
